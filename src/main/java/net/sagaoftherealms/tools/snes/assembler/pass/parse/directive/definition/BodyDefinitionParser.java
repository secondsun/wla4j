...
public abstract class BodyDefinitionParser extends GenericDirectiveParser {
    ...
    private Node makeUnionNode(SourceParser parser, Token token) {
        ...
        while (true) {
            ...
            switch (token.getType()) {
                case DIRECTIVE:
                    var bodyDirective = AllDirectives.valueOf(token.getString().replace(".", "").toUpperCase());
                    switch (bodyDirective) {
                        ...
                        case ENDU:
                            ...
                            return parentNode;
                        case NEXTU:
                            unionNode.setNextUnion((UnionNode) DirectiveUtils.createDirectiveNode(directive.getName(), token));
