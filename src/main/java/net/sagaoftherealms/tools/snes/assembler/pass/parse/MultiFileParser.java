  ...
  private boolean needsReparse(List<Node> fileNodes) {
    ...
    fileNodes =
        fileNodes.stream()
            ...
            .filter(
                fileNode ->
                    fileNode.getType().equals(NodeTypes.LABEL_DEFINITION)
    ...
    for (Node fileNode : fileNodes) {
      ...
      if (fileNode.getType().equals(NodeTypes.LABEL_DEFINITION)) {
        // Note : When we first parse a file macroNames that are from files that haven't been parsed
        // are resolved
        // as labels.  After parsing all files we look for labels that should be Macronames and
        // reparse those files
