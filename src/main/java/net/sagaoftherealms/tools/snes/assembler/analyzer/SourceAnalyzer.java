...
public class SourceAnalyzer {
  ...
  /**
   * This will scan an entire project, collect errors, etc. This will update all of the internal
   * state for the file and any files included in it.
   *
  ...
  private List<? extends ErrorNode> analyizeDirective(DirectiveNode node) {
    ...
    switch (node.getDirectiveType()) {
      ...
      case SNESNATIVEVECTOR -> {
        SNESNativeVectorAnalyzer mapAnal = new SNESNativeVectorAnalyzer(this.context);
        errors.addAll(mapAnal.checkDirective(node));
      }
